 ...
 */
export class FilterMapDivReduce extends MyAlgorithm {
  ...
  protected reduceList(): number {
    ...
    this.numberList.forEach((elem) => {
      reduceResult = elem / reduceResult;
    })
...
clientCode(new FilterMapDivReduce((x: number): boolean => { return x%2 === 0}, (x: number): number => {return x*2}, [1,2,3,4]));
